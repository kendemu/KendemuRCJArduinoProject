...
MyRobot2::MyRobot2(int no)
{
  if (no == 0) 	hmc6352 = 0;
  else          hmc6352 = 1;

  //デジタルコンパスモジュールのアドレス設定
  compassAddress = 0x42 >> 1;
  
  // digital pin number for Pings
  pingPin[0] = 10;
  pingPin[1] = 13;
  pingPin[2] = 30;
  pingPin[3] = 36;

  // analog pin for ball sensors
  ballSensorPin[0] = 1;
  ballSensorPin[1] = 2;
  ballSensorPin[2] = 3;
  ballSensorPin[3] = 4;
...
void MyRobot2::motorLF(double val) {
  int value = 2.55 * val;
...
void MyRobot2::motorLB(double val) {
  int value = 2.55 * val;
  if (value >=  255) value =  255;
  if (value <= -255) value = -255;
...
void MyRobot2::motorRF(double val) {
  int value = 2.55 * val;
  if (value >=  255) value =  255;
  if (value <= -255) value = -255;
...
void MyRobot2::motorRB(double val) {
  int value = 2.55 * val;
  if (value >=  255) value =  255;
  if (value <= -255) value = -255;
...
long MyRobot2::microsecondsToCentimeters(long microseconds)
{
...
void MyRobot2::moveForward(int power )
{
  // forward
  int offset = 5;
  motorLF(-power);
...
void MyRobot2::moveBack(int power)
{
  // forward
  motorLF(power);
  motorLB(power);
...
void MyRobot2::turnRight(int power )
{
  motorLF(-power);
  motorLB(-power);
  motorRF(-power);
...
void MyRobot2::turnLeft(int power)
{
  turnRight(-power);
}
...
unsigned int MyRobot2::getDir(unsigned char dno)
{
  if (hmc6352)
  {
    int reading = 999;
...
unsigned int MyRobot2::getPing(int pos)
{
...
unsigned int MyRobot2::getCompass()
{
  return getDir(0);
}

unsigned int MyRobot2::getBall(int pos)
{
