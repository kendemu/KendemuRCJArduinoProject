...
void movevec105(int vx , int vy , int spin ,int kick){ 
 ...
 m_r =   vx * sin(DEG2RAD(15))  - vy * cos(DEG2RAD(15))  - spin;
 m_b =  -vx * sin(DEG2RAD(270)) + vy * cos(DEG2RAD(270)) +  spin;
 m_l =  -vx * sin(DEG2RAD(165)) + vy * cos(DEG2RAD(165)) + spin;
...
double pid_control(){
  PID();
  double ang_vel = 40 * PI;//rad/s
  double accel = 2.963;
  double max_vel  = 6.28;
  double dif = -ang_vel * 0.21 * DEG2RAD(g_diff);
  double pro =  -DEG2RAD(g_diff) * 0.3;
  double integ = dif / (DEG2RAD(l_diff -  g_diff) * 50) ;
  if(l_diff - g_diff == 0)  integ = dif;
  return dif + pro + integ; 
}
